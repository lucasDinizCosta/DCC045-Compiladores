...
public class InterpretVisitor extends Visitor {
    ...
    public void visit(Function f) {
        ...
        // Qualquer retorno possivel da função estará no operands, portanto não tem
        // problema
    ...
    public void visit(NameType n) {
        try {
             ...
             * System.out.println("LINHA 282 --- " + r); operands.push(r); } else { // throw
             * new RuntimeException("Erro!"); throw new RuntimeException(" (" + n.getLine()
             * + ", " + n.getColumn() + ") " + " : Erro !!"); }
             */
        } catch (Exception x) {
    ...
    public void visit(Attribution a) {
        try {
            ...
            System.out.println("\n------------------------------------\n");
            a.getExp().accept(this); // Empilha o valor da expressão no operands
            debugMode();

            // Variavel que vai ter os dados atribuidos nela
            LValue lvalue = a.getLValue();

            // System.out.println("Linha 492 - ATTRIBUTION -- " + lvalue.getId() + " -- "
            //         + lvalue.getClass().getSimpleName() + " --- " + lvalue.toString());
            System.out.println("Linha 493 - ATTRIBUTION -- " + lvalue.getId() + " -- "
                    + lvalue.getClass().getSimpleName() + " --- " + a.toString());
            ...
            if (lvalue instanceof DataAccess) {
                ...
                if (((DataAccess) lvalue).getLValue() instanceof ArrayAccess) {
                    ...
                    // System.out.println("461 --- " + objetoArray.toString() + " -- " +
                    // nomeAtributo +" -- "+ valorAtribuicao);
                    ...
                    // if(((DataAccess)lvalue).getLValue() != null){ // Se o data tem um tipo data
                    // como atributo
                    // // nomeObjeto
                    // //((DataAccess)lvalue).getLValue().accept(this);
                    // ((DataAccess)lvalue).accept(this);
                    // //debugMode();
                    // //System.out.println("551 --- " + ((HashMap<String,
                    // Object>)operands.pop()).get("x"));
                    // }
                ...
                System.out.println("\n 566 ------ ARRAYACCESS ----- ATTRIBUTION\n");
                //lvalue.accept(this);        // Adiciona o vetor no operands e o elemento pra adicionar
                ((ArrayAccess)lvalue).getExp().accept(this); // Aceita e adiciona a posicao no operandos
                debugMode();
                System.out.println("569 -------------- " + operands);
                Integer position = (Integer)operands.pop();
                Object element = operands.pop();
                /*if(element instanceof TypeArray){   // Se é um tipo array o elemento
                    ((TypeArray)element).accept(this);  // Adiciona o vetor de array no operands
                    element = operands.pop();
                }*/
                List<Object> array = (List<Object>) operands.pop();
                System.out.println("570 ---- " + element + " ---- " + array +"---- " + position +  " ---- " +  operands);
                array.set(position, element);   // Adiciona no array pela referencia de memoria
                debugMode();
                /*ArrayAccess lvalueArray = ((ArrayAccess) lvalue);
                Boolean ehMatriz = false;
                Object linhaMatriz = null;

                if (lvalueArray.getLValue() != null) {
                    if (lvalueArray.getLValue() instanceof ArrayAccess) { // Array de array => Matriz
                        // lvalueArray.getLValue().accept(this); // Empilha o elemento da matriz
                        // ((ArrayAccess)lvalueArray.getLValue()).getExp().accept(this);
                        ehMatriz = true;
                        // lvalueArray.getExp().accept(this); // Empilha o indice da matriz
                        // ((ArrayAccess)lvalueArray.getLValue()).getExp().accept(this);
                        lvalueArray.getLValue().accept(this); // Me devolve um array na pilha de operands
                        linhaMatriz = operands.pop();
                        operands.pop();
                        System.out.println("571 --- " + operands + "---- " + lvalue + "---- " + lvalueArray.getExp()
                                + " ---- " + lvalueArray.getLValue());
                    }
                }
                ...
                // o array é um list de elementos
                String nomeArray = lvalueArray.getId();
                lvalueArray.getExp().accept(this); // Aceita e adiciona a posicao no operandos
                Integer position = (Integer) operands.pop(); // Posicao do array

                System.out.println("581 --- " + operands + "---- " + lvalue + "---- " + lvalue.getId() + "-----"
                        + lvalueArray.getExp() + " ---- " + lvalueArray.getLValue());
                // System.out.println(lvalueArray.);
                debugMode();
                ...
                */
                ...
                //          */
                //         // Integer tamanhoColunas = (Integer)env.peek().get(lvalueArray.getExp());
                //         System.out.println("597 --- " + linhaMatriz + " --- " + objetoArray);
                //         // Pega o elemento do topo de operands e adiciona na posição do vetor
                //         ((List) linhaMatriz).set(position, operands.pop());
                //         System.out.println("607 --- " + linhaMatriz + " --- " + objetoArray + " ---- " + operands);
            ...
            System.out.println("602 ------ FINAL DO ATTRIBUTION ----  ");
            debugMode();
        } catch (Exception x) {
    ...
    public void visit(TypeInstanciate t) {
        try {
            ...
            // Tipo comum: Int, Float, Char, Int[]...
            if (t.getType() != null) {
                if (t.getExp() != null) {           // Expressao do array
                    ...
                    if (t.getType() instanceof NameType) {
                        ...
                        Integer size = (Integer) operands.pop(); // Tamanho do array já foi visto
                        // System.out.println(i);
                        Object typeArray = operands.pop(); // Tipo do array -> Int, Float, Char ....
                        // System.out.println(obj.toString());
                        List<Object> lista = new ArrayList<Object>(size); // Tipo array
                        ...
                        // Pega o tamanho do vetor na pilha de operandos
                        System.out.println("\n998 -----------  TYPE INSTANCIATE\n");
                        debugMode();
                        Integer size = (Integer) operands.pop(); // Tamanho do array já foi visto
                        System.out.println("Tamanho do array: " + size);
                        Object typeArray = operands.pop(); // Tipo do array -> Int, Float, Char ....
                        System.out.println("Tipo do array instanciado: " + typeArray.toString());
                        List<Object> linhaArray = new ArrayList<Object>(size); // Tipo array
                        /*if(typeArray instanceof TypeArray){     // Cria a matriz
                            List<Object> colunaArray = new ArrayList<Object>(size); // Tipo array
                            for (int k = 0; k < size; k++) {
                                colunaArray = new ArrayList<Object>(size); // Tipo array
                                for (int i = 0; i < size; i++) {
                                    colunaArray.add(((TypeArray)typeArray).getType());
                                }
                                linhaArray.add(colunaArray);
                            }
                        }
                        else{
                            for (int k = 0; k < size; k++) {
                                linhaArray.add(typeArray);
                            }
                        }*/
                        for (int k = 0; k < size; k++) {
                            linhaArray.add(typeArray);
                        ...
                        /*
                         * if(typeArray instanceof TypeArray){ // (Matriz) Tipo de tipo de array for
                         * (int i = 0; i < size; i++) {
                         * 
                         * }
                         * 
                        ...
                        operands.push(linhaArray);
                        System.out.println("\n1108 ----------- FIM DE TYPE INSTANCIATE\n");
                    }
                ...
                if (t.getExp() == null) { // Tipo normal de data
                    ...
                    for (Declaration d : datas.get(dataID).getDeclarations()) {
                        ...
                        // Cria um objeto especial para destacar quais variaveis e seu tipo
                        // dentro do data
                        Object valorPadrao = new ObjectDefault(t.getLine(), t.getColumn(), d.getId(), d.getType());
        ...
        try {
            ...
            if (f != null) {
                ...
                if (function.getReturnTypes().size() == 2) {
                    if ((Integer) valueReturnedPos.getValue() == 0 || (Integer) valueReturnedPos.getValue() == 1) {
                    ...
                    throw new RuntimeException(
                            " (" + f.getLine() + ", " + f.getColumn() + ") A funcao nao apresenta tipos de retorno");
                }
            }
        } catch (Exception x) {
            throw new RuntimeException(" (" + f.getLine() + ", " + f.getColumn() + ") " + x.getMessage());
    ...
    public void visit(Identifier i) {
        try {
            // if (debug) {
            // Imprime a função
            System.out.println("\n -- Identifier " + i.getClass().getSimpleName());
            System.out.println(i.toString());
            System.out.println("\n");
            // }
            ...
            if (r != null || (r == null && env.peek().containsKey(i.getId()))) {
                ...
                throw new RuntimeException(
                        " (" + i.getLine() + ", " + i.getColumn() + ") " + ": Erro no Identifier !!");
            }
        } catch (Exception x) {
            throw new RuntimeException(" (" + i.getLine() + ", " + i.getColumn() + ") " + x.getMessage());
        }
    ...
    public void visit(ArrayAccess a) {
        try{
            ...
            if(a.getLValue() instanceof ArrayAccess){   //(MATRIZ) Se a variavel é um array com um array
                a.getLValue().accept(this);     // Executo o acesso a linha da matriz e empilho no operands
                System.out.println("1356 ------ " + a + " ---- " + operands);
                debugMode();
                a.getExp().accept(this);        // Adiciona a posicao da coluna no operands
                Object arrayLinha = operands.pop();    // Desempilha o vetor linha da matriz
                if(arrayLinha != null){
                    Integer position = (Integer) operands.pop();        // Desempilha a posição da coluna
                    Integer tamanhoColuna = ((List)arrayLinha).size();          // tamanho ta coluna
                    if((position >= 0) && (position <= tamanhoColuna - 1)){
                        System.out.println("1364 ----- " + arrayLinha + " ---- " + operands + " ---- " + position + " --- " 
                        + a.getLValue() + " ------ "+ a.getExp() + " ------ " + a.getLValue().getClass().getSimpleName());
                        //System.out.println("1309 ----- " + ((ArrayAccess)a.getLValue()).getExp());
                        //((ArrayAccess)a.getLValue()).getExp().accept(this);     // Empilha o indice da coluna
                        System.out.println("1368 ----- " + ((ArrayAccess)a.getLValue()).getExp() + " ---- ");
                        debugMode();
                        Object elementoNaMatriz = ((List)arrayLinha).get(position);
                        System.out.println("\n1311 ----- " + elementoNaMatriz + " ---- \n");
                        operands.push(elementoNaMatriz);
                    }else{
                ...
                if (obj != null) {
                    ...
                    Integer position = (Integer) operands.pop();
                    Object elemento = operands.pop();           // Elemento que sera adicionado no array
                    Integer tamanhoArray = ((List)obj).size();
                    if((position >= 0) && (position <= tamanhoArray - 1)){
                        System.out.println("1307 ----- " + obj + "- obj ---- " + operands + "-operands ---- " + position + "-position --- "  + a.getLValue() + "-- a.getLValue------ "+ a.getExp() + "-- a.getExp()------ " + a.getLValue().getClass().getSimpleName());
                        // operands.push(obj); // Empilha o array / matriz
                        operands.push(((List)obj).get(position));
                        debugMode();
                        System.out.println("\n1396 ----------------- FIM ARRAY-ACCESS\n");
                        // if(a.getLValue() instanceof ArrayAccess){   // É uma matriz
                        //     System.out.println("1309 ----- " + ((ArrayAccess)a.getLValue()).getExp());
                        //     ((ArrayAccess)a.getLValue()).getExp().accept(this);     // Empilha o indice da coluna
                        //     System.out.println("1311 ----- " + ((ArrayAccess)a.getLValue()).getExp() + " ---- ");
                        //     debugMode();
    
                        //     Object elementoNaMatriz = ((List)((List)obj).get(position)).get((Integer) operands.pop());
                        //     System.out.println("\n1311 ----- " + elementoNaMatriz + " ---- \n");
                        //     operands.push(elementoNaMatriz);
                        // }
                        // else{   // É um array comum
                            //operands.push(((List)obj).get(position));
                        // }
                        /*if(((ArrayAccess)obj).getLValue() instanceof ArrayAccess){ // matriz
                            System.out.println("1307 ----- " + obj);
                            if(lvalueArray.getLValue() != null){
                                if(lvalueArray.getLValue() instanceof ArrayAccess){ // Array de array => Matriz
                                    // ((ArrayAccess)lvalueArray.getLValue()).getExp().accept(this);
                                    ehMatriz = true;
                                    ...
                                    lvalueArray.getLValue().accept(this);   // Me devolve um array na pilha de operands
                                    linhaMatriz = operands.pop();
                                    System.out.println("571 --- " + operands+ "---- " + lvalue + "---- " + lvalueArray.getExp() + " ---- " + lvalueArray.getLValue());
                                }
                            }
                        }*/
                        // operands.push(((List)obj).get(position));
